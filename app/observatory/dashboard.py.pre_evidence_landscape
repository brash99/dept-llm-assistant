...
def render_observatory_assessment(st, assessment: ObservatoryAssessment) -> None:
    ...
    st.caption(
        ...
        "Knowledge completeness is stricter: it evaluates the empirical evidence set and is reduced when that "
        "evidence is planning-heavy, externally dependent, or thin in direct institutional records. "
        "Constitutional evidence is reported separately as institutional values."
    )
    ...
    r1.metric(
        ...
        _pct(assessment.constitutional_evidence_ratio),
    )
    r2.metric(
        "Institutional evidence",
        _pct(assessment.institutional_evidence_ratio),
    )
    r3.metric(
        "Planning reliance",
        _pct(assessment.planning_ratio),
    )
    r4.metric(
        "External dependence",
        _pct(assessment.external_ratio),
    )
    r5.metric(
        "Empirical balance",
        f"{assessment.evidence_balance_score:.0f}%",
    )
