...
PYTHONPATH="$PWD" pytest -q scripts/test_analytical_workforce.py scripts/test_analytical_workforce_review_matrix.py scripts/test_faculty_identity.py scripts/test_faculty_appointments.py scripts/test_authoritative_faculty_roster.py >"$RUN_ROOT/tests.log"
for run in 1 2; do
...
PYTHONPATH="$PWD" python scripts/a100_testing_scripts/build_analytical_workforce_review_matrix.py --run-root "$RUN_ROOT" >"$RUN_ROOT/review_matrix.log"
